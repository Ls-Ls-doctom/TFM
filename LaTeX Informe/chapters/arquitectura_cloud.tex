...
\textbf{INE · SEPE · datos abiertos municipales}\\[0.25cm]
$\downarrow$ extracción programada\\[0.2cm]
\fbox{\parbox{0.72\textwidth}{\centering ECS Fargate · contenedor del pipeline · EventBridge mensual}}\\[0.2cm]
$\downarrow$ Parquet y metadatos\\[0.2cm]
\fbox{\parbox{0.72\textwidth}{\centering Amazon S3 · Bronze $\rightarrow$ Silver $\rightarrow$ Gold}}\\[0.2cm]
$\downarrow$ catálogo y consulta\\[0.2cm]
\fbox{\parbox{0.72\textwidth}{\centering AWS Glue $\rightarrow$ Amazon Athena $\rightarrow$ AWS Lambda}}\\[0.2cm]
$\downarrow$ API HTTPS\\[0.2cm]
\fbox{\parbox{0.72\textwidth}{\centering Vercel · dashboard y chatbot · Gemini 2.5 Flash}}
}}
...
La ejecución puede dispararse manualmente mediante la CLI de AWS y se encuentra programada mediante Amazon EventBridge Scheduler el día 1 de cada mes a las 07:00, zona \texttt{Europe/Madrid}, con dos reintentos. El sistema opera en la región \texttt{eu-west-1} (Irlanda), usando subredes públicas e IP pública para alcanzar las APIs externas sin asumir el coste fijo de una pasarela NAT; el contenedor no publica puertos de entrada.
...
ECS Fargate & 1 ejecución mensual, hasta 2 h, 1 vCPU / 4 GB & $\sim$ 0,12 USD \\
...
\midrule
\textbf{Infraestructura estimada} & Excluye consumo variable de Gemini & \textbf{$<$ 0,30 USD/mes} \\
\bottomrule
